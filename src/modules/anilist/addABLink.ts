import { $, waitFor, createElement, removeElements } from '@/utils/Helpers';
import { registerModule } from '@/utils/ModuleLoader';

registerModule.anilist({
	id: 'addABLink',

	validate({ pathname }) {
		return /\/(anime|manga)\/\d+/.test(pathname);
	},

	async load({ media }) {
		const targetLoaded = await waitFor('.sidebar');

		// If the target element or mal id is not found, return.
		if (!targetLoaded || !media?.malId) return;

		let attrName: string;

		// In some cases this element may exist. We will wait/check to see if it
		// does and if not we will create it ourselves.
		if (await waitFor('.external-links .external-links-wrap')) {
			const attrEl = $('.external-links .external-links-wrap > a');
			attrName = attrEl!.attributes[0].name;
		} else {
			// Setting the "data-v-" attribute manually is not ideal as
			// this could change in the future but it'll do for now.
			attrName = 'data-v-c1b7ee7c';

			createElement('div', {
				attributes: {
					[attrName]: '',
					class: 'external-links alextras--external-links',
				},
				children: [
					createElement('h2', {
						textContent: 'External Links',
					}),
					createElement('div', {
						attributes: {
							class: 'external-links-wrap',
						},
					}),
				],
				appendTo: $('.sidebar')!,
			});
		}

		const animeTitle = $('.header .content h1')!.textContent;

		if (!animeTitle) return;

		createElement('a', {
			attributes: {
				[attrName]: '',
				class: 'external-link alextras--ab-link',
				target: '_blank',
				href: `https://animebytes.tv/torrents.php?filter_cat%5B1%5D=1&searchstr=${encodeURIComponent(animeTitle)}&action=advanced&search_type=title&sort=relevance&way=desc&hentai=2&showhidden=0`,
			},
			styles: {
				'--link-color': '#e7116c',
			},
			children: [
				createElement('div', {
					attributes: {
						[attrName]: '',
						class: 'icon-wrap',
					},
					styles: {
						padding: '0px',
						background: '#e7116c',
					},
					children: [
						createElement('img', {
							attributes: {
								[attrName]: '',
								class: 'icon',
								src: 'https://pbs.twimg.com/profile_images/1079461625982267398/AEGkeD97_400x400.jpg',
							},
							styles: {
								'border-radius': '5px',
							},
						}),
					],
				}),
				createElement('div', {
					attributes: {
						[attrName]: '',
						class: 'name',
					},
					textContent: 'AnimeBytes Search',
				}),
			],
			appendTo: $('.external-links .external-links-wrap')!,
		});
	},

	unload() {
		removeElements('.alextras--ab-link, .alextras--external-links');
	},
});
